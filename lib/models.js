...
async function call(role, messages, {
  ...
  return {
    ...
    content: (j.choices?.[0]?.message?.content || j.choices?.[0]?.message?.reasoning || '').trim(),
    finishReason: j.choices?.[0]?.finish_reason || null,
    rawChoice: (j.choices?.[0]?.message?.content ? null : JSON.stringify(j.choices?.[0] || j).slice(0, 400)),
