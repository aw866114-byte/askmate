...
const EXEC = {
  async web_search({ query }) {
    ...
    return t.slice(0, 4000) || 'no results';
  },
  async web_fetch({ url }) {
  ...
  async github_read({ repo, path }) {
    ...
    return Buffer.from(j.content || '', 'base64').toString('utf8').slice(0, 12000);
  },
  async github_write({ repo, path, content, message = 'askmate' }) {
    if (!process.env.GITHUB_TOKEN) return 'no GITHUB_TOKEN set';
    ...
    return r.ok ? `written ${path} — Vercel will redeploy` : `HTTP ${r.status}: ${(await r.text()).slice(0,200)}`;
  },
