...
const DEFS = [
  ...
  { type:'function', function:{ name:'local_job', description:"Give a job to the agent running on AJ's OWN COMPUTER. This is the ONLY way to touch his disk - AskMate itself cannot. Best use: copy a site folder, change some text inside the COPY, and zip it into his Downloads for him to drag. It NEVER deletes, NEVER writes over anything that already existed, and NEVER deploys. Send the verb first, then ONE LINE of JSON. Verbs: list, count, copy, write, zip, md5, copyfile, build, find, edit, run, allow. run takes cmd and in, and it ONLY runs a program that is on AJ's own written list in allowed_commands.txt - anything else is refused out loud and nothing happens. That list is how his reels get built, for example run with cmd python build_week_20260817.py and in REEL_ENGINE. allow ADDS a line to that list, taking name, cmd and in, so AJ can add jobs himself without asking Claude; allow with show set true reads the whole list back to him. find {\"what\":\"cheryl\",\"where\":\"C:/Users/rippe/Downloads\",\"inside\":true} searches names and, with inside, the text in files. edit {\"file\":\"...\",\"find\":\"...\",\"replace\":\"...\",\"count\":1} changes an existing file after saving the original beside it. Example: build {\"src\":\"C:/Users/rippe/OneDrive/Documents/ajws Empire/claude/allcare-DEPLOY-READY\",\"dst\":\"C:/Users/rippe/OneDrive/Documents/ajws Empire/claude/allcare-build-2026-08-19\",\"zip\":\"C:/Users/rippe/Downloads/allcare-2026-08-19.zip\",\"edits\":[{\"file\":\"index.html\",\"find\":\"10 reviews\",\"replace\":\"11 reviews\",\"count\":1}]}. It only runs when his computer is on and the agent is running, so say that when you use it.",
...
const EXEC = {
  ...
  async verifymate_read({ bucket = 'settled', search = '' }) {
    ...
    const r = await fetch(`${base}/api/context?full=1`, { headers:{ Authorization:`Bearer ${process.env.VERIFYMATE_AGENT_KEY}` } });
  ...
  async local_job({ task }) {
    ...
    const KNOWN = ['list','count','copy','write','zip','md5','copyfile','build','find','edit','run','allow'];
    ...
    // 19 Aug 2026. This used to say "queued" whether or not AJ's computer was even on,
    // which is a confident-sounding lie. Now it asks. The agent heartbeats every 20 s.
    let live = null;
    try {
      const base = process.env.ASKMATE_URL || 'https://askmate-umber.vercel.app';
      const h = await fetch(base + '/api/claim', { headers: { Authorization: `Bearer ${process.env.ASKMATE_KEY}` } });
      if (h.ok) live = await h.json();
    } catch (e) { /* if the check itself fails, say we do not know rather than guess */ }
    const alive = live ? live.anyAlive : null;
    const who = live && (live.agents || []).filter(a => a.alive).map(a => a.name);
    return JSON.stringify({ queued: true, id: (r && r.id) || null, verb,
      computerListening: alive,
      agents: who && who.length ? who : undefined,
      note: alive === true
        ? `His computer IS listening (${(who || []).join(', ')}). This runs within about 20 seconds and it will say the answer out loud. Do not claim it is done until he says he heard it.`
        : alive === false
          ? "His computer is NOT listening right now - it is off, or the agent is not running. The job is safely queued and will run the moment he turns it on. TELL HIM THAT plainly instead of saying it is done."
          : "Could not tell whether his computer is listening. Say so honestly - the job is queued either way." });
  },
