...
const KEEP_TURNS = 12;     // six of his, six of its. Enough for "do that again". Not enough to cost money.
const MAX_CHARS  = 7000;   // a hard ceiling, so one huge paste cannot slow every later press.
...
async function remember(thread, said, replied) {
  try {
    ...
    if (said)    turns.push({ who: 'AJ',      text: String(said).slice(0, 2500) });
    if (replied) turns.push({ who: 'AskMate', text: String(replied).slice(0, 2500) });
